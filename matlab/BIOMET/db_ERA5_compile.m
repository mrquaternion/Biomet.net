...
% Currently set to do unit conversions for: dew point temperature (d2m - °K));
%   air temperature (t2m - °K); incoming shortwave radiation (ssrd - J hr^-1 m^-2);
...
varStr_nc = ERA5_vars.Short_name;
varStr_file = ERA5_vars.Full_name;
...
if isfield(yml_data,'Metadata')
    ...
    if isfield(yml_data.Metadata,'TimeZoneHour')
        GMT_offset_yml = yml_data.Metadata.TimeZoneHour;
    end
end

if isempty(lat_target) | isempty(lon_target) | isempty(GMT_offset_yml)
    ...
    if ~isempty(files)
        for j=1:length(files)
            ...
            if contains(unitStr,'hours')
                ...
                CF = 24;
                GMT_offset_nc = GMT_offset_yml;
            else
                % Seconds since 1970-01-01
                GMT_offset_nc = GMT_offset_yml .* 3600;
                CF = 86400; % Conversion factor
            end
            
            tv = ncread(source,'valid_time') + GMT_offset_nc;
